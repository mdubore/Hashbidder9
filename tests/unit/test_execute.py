...
class TestRetries:
    ...
    async def test_transient_error_retries_then_succeeds(self) -> None:
        ...
        plan = plan_bid_changes(config, await client.get_current_bids())

        result = await execute_plan(client, plan, sleep=_no_sleep)
...
class TestContinueOnFailure:
    ...
    async def test_failure_does_not_block_subsequent_actions(self) -> None:
...
class TestAtomicUpstreamPairs:
    ...
    async def test_cancel_fails_skips_linked_create(self) -> None:
    ...
    async def test_cancel_succeeds_create_fails_no_rollback(self) -> None:
        ...
        result = await execute_plan(failing_client, plan, sleep=_no_sleep)
    ...
    async def test_unmatched_cancel_failure_does_not_skip_creates(self) -> None:
...
class TestExecutionOrder:
    ...
    async def test_cancels_before_edits_before_creates(self) -> None:
...
class TestRefetchDelay:
    ...
    async def test_sleeps_before_refetch_when_plan_has_actions(self) -> None:
        ...
        await execute_plan(client, plan, sleep=_capture_sleep)

        assert POST_EXECUTE_REFETCH_DELAY_SECONDS in sleeps

    async def test_no_sleep_when_plan_is_empty(self) -> None:
